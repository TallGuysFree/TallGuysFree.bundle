...
APIURL 							= 'https://api.tallguysfree.com/go.php'
HTTP.CacheTime 					= 0
...
def MainMenu():
	...
	try:
		categories = JSON.ObjectFromURL(APIURL)
	except:
		...
		if len(categories) == 0:
			...
			for category in categories:
				oc.add(DirectoryObject(
					#key = Callback(GetChildrenOfCategory, title=item_title, categoryId=1),
					title = category['item_title'],
